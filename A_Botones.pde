...
void huevo() {
 ...
 juego.reset();
 juego.HuevosPorEstado();
 juego.Ganar();
  /*image(carga.fondos[2], 0, 0, width, height);
  text(texto.textos[8], 30, 40);
  botonDibujado(310, 50, 150, 50);//cocina
  text(texto.textos[7], 280, 40);
  botonDibujado(50, 50, 150, 50);//cae huevo
  pushStyle();
  textSize(15);
  fill(0);
  text(texto.textos[3], 200, 250);
  popStyle();*/
}
void llave() {
  ...
  text(texto.textos[6], 330, 255);//ventana(texto)
